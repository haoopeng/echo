var textSet = {
    "italiano": {
        ...
        "InfoRepresentA": "Ciascun nodo nella rete rappresenta un utente; la sua grandezza rappresenta la sua popolarità (numero di amici) e il suo colore rappresenta la sua opinione politica. I collegamenti tra amici sono da considerare reciproci.",
        ...
        "InfoHowA": "Gli utenti fanno quattro cose: (1) leggo i message postati/ri-postati dai loro amici; (2) cambiano le proprie opinioni un po' (in base al parametro di influenza) in seguito alla lettura dei messagi; (3) ogni tanto (in base al parametro di unfriending) sostituiscono un amico che posta messaggi sufficientemente differenti dalla loro opinione (in base al parametro della tolleranza) con un amico scelto a caso; e (4) postano un nuovo messaggio che riflette la loro opinione, o ripostano un message da un amico scelto a caso.",
    ...
    "japanese": {
        ...
        "InfoHowA": "ユーザは次の4つのことをします。(1)ユーザは友人が投稿・再投稿したメッセージ読みます。(2)ユーザはこれらのメッセージから影響を受けて、自分の意見を(社会的影響パラメータに基づいて)少しだけ変えます。(3)(アンフォローの頻度のパラメータに基づいて)時々、ユーザは自分の意見とは少し異なる意見のメッセージを投稿した友人をアンフォローし、別の友人をランダムに選んでフォローします。(4)ユーザは自分の意見を反映した新しいメッセージを投稿するか、ランダムに選んだ友人のメッセージを再投稿します。",
    ...
    "chinese": {
        ...
        "High": "高",
        "ToleranceDiscription": "每条消息内在是带有观点的。该参数定义了用户对于不同观点的容忍度。我们把与当前主体观点相近的消息称为一致消息，否则为不一致消息。一致消息会影响当前主体的观点。同时，当前主体可能会取消关注发布不一致消息的已关注用户。",
        "InfoHowA": "",
    ...
    "english": {
        ...
        "InfoHowA": "Users do four things: (1) they read messages posted/re-posted by their friends; (2) they change their opinion a bit (based on the influence parameter) according to these messages; (3) once in a while (based on the unfriending parameter) they replace a friend who posts messages sufficiently different from their own opinion (based on the tolerance parameter) with a random friend; and (4) they either post a new message reflecting their opinion, or re-post a message from a random friend.",
